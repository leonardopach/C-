...
namespace EnumExercicio {
    ...
    class Programa {
        static void Main(string[] args) {
            ...
            WorkerLevel level = Enum.Parse<WorkerLevel>(Console.ReadLine() ?? "0");
            Console.Write("Base salary: ");
            double baseSalary = Convert.ToDouble(Console.ReadLine(), CultureInfo.InvariantCulture);

            Department dept = new Department(deptName ?? "zero");
            ...
            for (int i = 1; i <= n; i++) {
                ...
                DateTime date = DateTime.Parse(Console.ReadLine() ?? "0");
            ...
            int month = Convert.ToInt16(monthAndYear?.Substring(0, 2));
            int year = Convert.ToInt16(monthAndYear?.Substring(3));
